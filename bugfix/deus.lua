...
local using_hex_or_256 = tonumber(vim.go.t_Co) >= 256
	or vim.go.termguicolors
...
if not using_hex_or_256 then vim.go.t_Co = 16 end
...
local function highlight(highlight_group, attributes) -- {{{ †
	...
	if attributes[vim.go.background] then
		...
		attributes = setmetatable(attributes[vim.go.background], attributes)
	end
